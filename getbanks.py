...
def TripleDES_encrypt(account_number: str, cbn_bankcode: str, secret_key: str) -> str:
    """
    Encrypt account_number and cbn_bankcode using TripleDES encryption.
    
    Args:
        account_number (str): The account number to encrypt
        cbn_bankcode (str): The CBN bank code to encrypt
...
def TripleDES_decrypt(encrypted_text: str, secret_key: str) -> str:
    ...
    try:
        ...
        decrypted_text = unpad(decrypted_padded, DES3.block_size).decode('utf-16le')
...
class Transaction(BaseModel):
    ...
    transaction_ref_parent: Optional[str] = None
    amount: int
    customer: Customer
    meta: Dict[str, Any] = {}
...
class GetBanksPayload(BaseModel):
    account_number: str
    cbn_bankcode: str
    request_ref: str
    request_type: str = "lookup_account_min"
...
async def get_banks(payload: GetBanksPayload):
    """
    Get banks endpoint that encrypts account details and calls OnePipe API for account lookup.
    
    Args:
        payload: GetBanksPayload containing account details and transaction info
